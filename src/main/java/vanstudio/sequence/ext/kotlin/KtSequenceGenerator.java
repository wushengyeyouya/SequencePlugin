...
public class KtSequenceGenerator extends KtTreeVisitorVoid implements IGenerator {
    ...
    private MethodDescription createMethod(PsiElement psiElement, int offset) {
        ...
        else if (psiElement instanceof PsiMethod) return createMethod((PsiMethod) psiElement, offset);
    ...
    private MethodDescription createMethod(KtSecondaryConstructor constructor, int offset) {
        return getMethodDescription(offset, constructor.getValueParameters(), constructor.getContainingClassOrObject(), constructor.getModifierList(), constructor.getName());
    }
    ...
    @NotNull
    private MethodDescription getMethodDescription(int offset, List<KtParameter> valueParameters, KtClassOrObject containingClassOrObject, KtModifierList modifierList, String name) {
        ParamPair paramPair = extractParameters(valueParameters);
        ClassDescription classDescription = new ClassDescription(containingClassOrObject.getFqName().asString(), new ArrayList<>());

        List<String> attributes = createAttributes(modifierList);
